...
require_once 'models/Inscripcion.php'; // Se añade el modelo de Inscripción
...
class GrupoController extends BaseController {
    ...
    private $inscripcionModel; // Se añade el modelo de Inscripción
    
    public function __construct() {
        $this->grupoModel = new Grupo();
        $this->periodoModel = new Periodo();
        $this->materiaModel = new Materia();
        $this->inscripcionModel = new Inscripcion(); // Se instancia el modelo
    }
    
    // ... el método index() y create() permanecen igual ...
    ...
    public function create() {
        try {
            ...
            $this->error('Error al preparar el formulario de creación: ' . $e->getMessage());
        }
    }
    public function store() {
        ...
        $data = [
            ...
            'periodo_id' => $this->post('periodo_id'),
        ...
        try {
            ...
            if ($id) {
                $this->setFlash('success', 'Grupo creado exitosamente.');
                $this->redirect(BASE_URL . '/grupos');
            } else {
                $this->setFlash('error', 'No se pudo crear el grupo.');
                $this->redirect(BASE_URL . '/grupos/crear');
            }
        } catch (Exception $e) {
    ...
    // ... los métodos edit(), update() y delete() permanecen igual ...
    public function edit($id) {
        try {
            ...
            $this->setFlash('error', 'Error al cargar el grupo: ' . $e->getMessage());
            $this->redirect(BASE_URL . '/grupos');
        }
    }
    public function update($id) {
        ...
        try {
            ...
            if ($success) {
                $this->setFlash('success', 'Grupo actualizado exitosamente.');
                $this->redirect(BASE_URL . '/grupos');
            } else {
                $this->setFlash('error', 'No se pudo actualizar el grupo.');
                $this->redirect(BASE_URL . "/grupos/editar/{$id}");
            }
        } catch (Exception $e) {
    ...
    public function delete($id) {
        ...
        try {
            ...
            if ($success) {
                $this->setFlash('success', 'Grupo eliminado exitosamente.');
            } else {
                $this->setFlash('error', 'No se pudo eliminar el grupo. Es posible que tenga estudiantes inscritos.');
            }
        } catch (Exception $e) {
    ...
    public function quitarInscripcion($inscripcion_id) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Obtener el grupo_id antes de borrar, para saber a dónde redirigir
